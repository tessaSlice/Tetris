...
public class Block implements Constants {
	...
	public boolean rotateCCW(Game game) {
		...
		for (int i = 0; i < coordinates.size(); i++) {
			if (intersectsOtherBlock(game, 3))
				return false; // check to see if it rotates
		}
		// if it doesn't intersect... then actually reset it
		for (int i = 0; i < coordinates.size(); i++) {
			int x = coordinates.get(i).x + BLOCK_SIZE / 2; // shift them because they're currently on the top left
															// corner
			int y = coordinates.get(i).y + BLOCK_SIZE / 2; // shift them because they're currently on the top left
															// corner
	...
	public boolean shiftRight(Game game) {
		...
		for (int i = 0; i < coordinates.size(); i++) {
			tempXs[i] = (int) (coordinates.get(i).getX() + BLOCK_SIZE);
			if (tempXs[i] > game.coordX + WIDE - BLOCK_SIZE)
				return false; // just stops the operation from happening
			if (intersectsOtherBlock(game, 2))
				return false;
		}
		int tempY;
		for (int i = 0; i < coordinates.size(); i++) {
	...
	public boolean shiftDown(Game game) {
		...
		// also check to see if it can't move down anymore... then return a boolean
		// probably or else it will never set it to finished or set the new value, etc.
		...
		for (int i = 0; i < coordinates.size(); i++) {
			...
			;
			if (intersectsOtherBlock(game, 0))
				return false;
		}
		for (int i = 0; i < coordinates.size(); i++) {
	...
	public void hold(Game game) {
		if (this.held != 0)
			return; // can't just spam repeat holds... can only hold once per block
		...
		// reset coordinates based on posy and posx gah! has to make it block specific
		// too!
	...
	public boolean intersectsOtherBlock(Game game, int condition) {
		...
		// condition == 3 for CCW, condition == 4 if it's literally in the same location
				...
				for (int j = 0; j < game.blocks.get(i).coordinates.size(); j++) {
					for (int k = 0; k < this.coordinates.size(); k++) {
						if (condition == 1) {
							...
							if (game.blocks.get(i).finished
									&& game.blocks.get(i).coordinates.get(j).getX() == (this.coordinates.get(k).getX())
											- BLOCK_SIZE
									&& game.blocks.get(i).coordinates.get(j).getY() == (this.coordinates.get(k).getY()))
								doesIntersect = true;
						} else if (condition == 2) {
							if (game.blocks.get(i).finished
									&& game.blocks.get(i).coordinates.get(j).getX() == (this.coordinates.get(k).getX())
							...
							// it's just checking downwards, has to make sure that the block is on the
							// ground first
							if (game.blocks.get(i).finished
									&& game.blocks.get(i).coordinates.get(j).getX() == (this.coordinates.get(k).getX())
									&& game.blocks.get(i).coordinates.get(j)
											.getY() == (this.coordinates.get(k).getY() + BLOCK_SIZE))
								doesIntersect = true;
						} else if (condition == 3) {
							// checks for CCW rotation
							int x = coordinates.get(k).x + BLOCK_SIZE / 2; // shift them because they're currently on
																			// the top left corner
							int y = coordinates.get(k).y + BLOCK_SIZE / 2; // shift them because they're currently on
																			// the top left corner
							int newX = x - (int) rotationPoint.getX();
							int newY = y - (int) rotationPoint.getY();
							int newPtX = -newY;
							...
							newPtX -= BLOCK_SIZE / 2;
							newPtY -= BLOCK_SIZE / 2;
							if (game.blocks.get(i).coordinates.get(j).getX() == newPtX
									&& game.blocks.get(i).coordinates.get(j).getY() == newPtY)
								doesIntersect = true;
							if (newPtX < game.coordX || newPtX > game.coordX + WIDE - BLOCK_SIZE) { // check to see if
																									// it also is
																									// outside of the
																									// left boundary
								doesIntersect = true; // got kinda lazy, could do a shifting move first and then rotate
														// but then I'll have to check again... gah!
							}
						} else {
